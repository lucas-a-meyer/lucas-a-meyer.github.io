...
This seemingly simple problem is harder than it looks. People don't stare at urinals for several minutes, but that can happen to computer screens (e.g., watching a movie or presentation, reading a document). Also, people usually stand in front of urinals and sit on toilets, but you can use computers from several different vantage points. 

For example, my computer is to my left at my home office, and to my right at my office. In both cases I'm using an external keyboard and monitor, and I probably wouldn't want to add an infrared detector in each place just so my computer would automatically lock.

Newer phones and tablets use eye tracking to save battery. It works well, but these are devices that you almost always use directly, so they can keep a camera pointing at you all the time. If you cover the camera or use an external monitor, you start having problems.
